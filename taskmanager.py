...
class StartScreen:
    def __init__(self):
        ...
        welcome_label = Label(self.main_frame, text="Welcome, %s" % username, font=('Arial', 17), bg="#333", fg="#efefef")
        ...
        date_label = Label(self.main_frame, text="%s %s %s" % (self.now.day, month_dict[self.now.month], self.now.year), font=('Arial', 15), bg="#333", fg="#efefef")
        ...
        totd_label = Label(self.main_frame, text="Quote of the Day", font=('Arial', 17), bg="#333", fg="#efefef")
        quote_label = Label(self.main_frame, text="%s" % quote, font=('Arial', 15), bg="#333", fg="#efefef")
...
class ToDoList:
    def __init__(self):
        ...
        year_label = Label(left_frame, text="%s Resolutions" % str(self.now.year), font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")
        self.yearlies_listbox = Listbox(left_frame, width=30, height=24, font=('Arial', 15), justify=CENTER, exportselection=False)
        ...
        dailies_label = Label(center_frame, text="Dailies", font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")
        self.dailies_listbox = Listbox(center_frame, width=38, height=22, font=('Arial', 15), justify=CENTER, exportselection=False)
        ...
        width = 9
        font_size = 12
        add_button = Button(task_button_frame, text="Add", width=width, font=('Helvetica', font_size))
        edit_button = Button(task_button_frame, text="Edit", width=width, font=('Helvetica', font_size))
        remove_button = Button(task_button_frame, text="Remove", width=width, font=('Helvetica', font_size))
        clear_button = Button(task_button_frame, text='Clear All', width=width, font=('Helvetica', font_size))
        ...
        goals_label = Label(goal_frame, text="Goals", font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")
        self.goals_listbox = Listbox(goal_frame, width=goals_listbox_width, height=18, font=('Arial', 15), justify=CENTER, exportselection=False)
        ...
        deadline_label = Label(goal_frame, text="Deadline", font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")
        self.deadline_listbox = Listbox(goal_frame, width=45 - goals_listbox_width, height=18, font=('Arial', 15), justify=CENTER)
        ...
        self.task_info = Text(task_info_frame, width=43, height=7, font=('Helvetica', 12))

        task_info_frame.grid(row=2, columnspan=2)
        self.task_info.grid(row=0, padx=(0,20), sticky=N+W)

        info_button_frame = Frame(task_info_frame, bg="#333")
        save_info_button = Button(info_button_frame, text="Save", width=width, font=('Helvetica', font_size))
        clear_info_button = Button(info_button_frame, text="Clear", width=width, font=('Helvetica', font_size))
        self.save_indicator_label = Label(info_button_frame, text="", font=('Helvetica', 12, 'bold'), bg="#333", fg="#efefef")

        save_info_button.bind('<Button-1>', self.save_task_info)
        clear_info_button.bind('<Button-1>', self.clear_task_info)

        info_button_frame.grid(row=0, column=1, sticky=N)
    ...
    def initialize(self):
        ...
        # Yearlies
        self.db.raw_execute("SELECT taskName FROM yearList ORDER BY taskId ASC")
        ...
        # Dailies
        self.db.raw_execute("SELECT taskName FROM dailiesList ORDER BY taskId ASC")
        all_tasks = self.db.fetchall()
        task_list = []
        for task in all_tasks:
            task_list.append(str(task[0]))
        ...
        # Goals
        self.db.raw_execute("SELECT taskName, deadline FROM goalsList ORDER BY taskId ASC")
        all_tasks = self.db.fetchall()
        task_list = []
        for task in all_tasks:
            if len(task) > 0:
    ...
    def select_yearlies(self, event=""):
        ...
        try:
            ...
            self.db.select('taskInfo', 'taskInfo', "taskName = '%s' AND listId = 0" % (self.tool.to_database(task_name)))
    ...
    def select_dailies(self, event=""):
        ...
        try:
            ...
            self.db.select('taskInfo', 'taskInfo', "taskName = '%s' AND listId = 1" % (self.tool.to_database(task_name)))
    ...
    def select_goals(self, evenet=""):
        ...
        try:
            ...
            self.db.select('taskInfo', 'taskInfo', "taskName = '%s'AND listId = 2" % (self.tool.to_database(task_name)))
            task_info = self.db.fetchone()[0]
            self.task_info.insert(END, task_info)
        except Exception as e:
            pass
    ...
    def add_task(self, event=""):
        ...
        add_yearlies_button = Button(self.center_frame, text="Yearlies", width=10)
        add_dailies_button = Button(self.center_frame, text="Dailies", width=10)
    ...
    def add_yearlies(self, event=""):
        ...
        add_yearlies_label = Label(self.add_task_popup, text="Enter Yearly Name", bg="#333", foreground="#efefef", font=('Arial', 13))
        self.add_yearlies_entry = Entry(self.add_task_popup, width=40, justify=CENTER)
        add_yearlies_button = Button(self.add_task_popup, text="Add Yearly", width=18)
    ...
    def add_yearlies_to_app(self, event=""):
        ...
        if len(task_name) > 0:
            if max_id == None:
                self.db.insert('yearList', 'taskId, taskName', " 1, '%s'" % (self.tool.to_database(task_name)))
            else:
                self.db.insert('yearList', 'taskId, taskName', "%d, '%s'" % (max_id+1, self.tool.to_database(task_name)))
    ...
    def add_dailies(self, event=""):
        ...
        add_dailies_label = Label(self.add_task_popup, text="Enter Daily Name", bg="#333", foreground="#efefef", font=('Arial', 13))
        self.add_dailies_entry = Entry(self.add_task_popup, width=40, justify=CENTER)
        add_dailies_button = Button(self.add_task_popup, text="Add Daily", width=18)
    ...
    def add_dailies_to_app(self, event=""):
        ...
        if len(task_name) > 0:
            if max_id == None:
                self.db.insert('dailiesList', 'taskId, taskName', " 1, '%s'" % (self.tool.to_database(task_name)))
            else:
                self.db.insert('dailiesList', 'taskId, taskName', "%d, '%s'" % (max_id+1, self.tool.to_database(task_name)))
    ...
    def add_goals(self, event=""):
        ...
        width = 40
        add_goals_label = Label(self.add_task_popup, text="Enter Goal Name", bg="#333", foreground="#efefef", font=('Arial', 13))
        self.add_goals_entry = Entry(self.add_task_popup, width=width, justify=CENTER)
        add_deadline_label = Label(self.add_task_popup, text="Enter Deadline Date", bg="#333", foreground="#efefef", font=('Arial', 13))
        self.add_deadline_entry = Entry(self.add_task_popup, width=width, justify=CENTER)

        add_goals_button = Button(self.add_task_popup, text="Add Goal", width=18)
    ...
    def add_goals_to_app(self, event=""):
        ...
        if len(task_name) > 0:
            if max_id == None:
                self.db.insert('goalsList', 'taskId, taskName, deadline', " 1, '%s', '%s'" % (self.tool.to_database(task_name), self.tool.to_database(deadline)))
            else:
                self.db.insert('goalsList', 'taskId, taskName, deadline', "%d, '%s', '%s'" % (max_id+1, self.tool.to_database(task_name), self.tool.to_database(deadline)))
    ...
    def rename_task(self, event):
        if self.list_id == 0:
            ...
            rename_yearly_label = Label(self.rename_task_popup, text="Rename %s to" % task_name, bg="#333", foreground="#efefef", font=('Arial', 13))
            self.rename_yearly_entry = Entry(self.rename_task_popup, width=width, justify=CENTER)
            self.rename_yearly_entry.insert(0, task_name)
            rename_yearly_button = Button(self.rename_task_popup, text="Rename Yearly", width=20)
            ...
            rename_daily_label = Label(self.rename_task_popup, text="Rename %s to" % task_name, bg="#333", foreground="#efefef", font=('Arial', 13))
            self.rename_daily_entry = Entry(self.rename_task_popup, width=width, justify=CENTER)
            self.rename_daily_entry.insert(0, task_name)

            rename_daily_button = Button(self.rename_task_popup, text="Rename Daily", width=20)
            ...
            rename_goals_label = Label(self.rename_task_popup, text="Enter Goal Name", bg="#333", foreground="#efefef", font=('Arial', 13))
            self.rename_goals_entry = Entry(self.rename_task_popup, width=width, justify=CENTER)
            self.rename_goals_entry.insert(END, task_name)
            rename_deadline_label = Label(self.rename_task_popup, text="Enter Deadline Date", bg="#333", foreground="#efefef", font=('Arial', 13))
            self.rename_deadline_entry = Entry(self.rename_task_popup, width=width, justify=CENTER)
            self.rename_deadline_entry.insert(END, deadline)

            rename_goals_button = Button(self.rename_task_popup, text="Rename Goal", width=18)
    ...
    def rename_task_to_app(self, event):
        ...
        if self.list_id == 0:
            ...
            if len(renamed_task_name) > 0:
                self.db.update('yearList', "taskName = '%s'" % self.tool.to_database(renamed_task_name), "taskName = '%s'" % self.tool.to_database(task_name))
                self.db.update('taskInfo', "taskName = '%s'"% self.tool.to_database(renamed_task_name), "taskName = '%s' AND listId = 0" % self.tool.to_database(task_name))
            ...
            if len(renamed_task_name) > 0:
                self.db.update('dailiesList', "taskName = '%s'" % self.tool.to_database(renamed_task_name), "taskName = '%s'" % self.tool.to_database(task_name))
                self.db.update('taskInfo', "taskName = '%s'"% self.tool.to_database(renamed_task_name), "taskName = '%s' AND listId = 1" % self.tool.to_database(task_name))
            ...
            if len(renamed_task_name) > 0:
                self.db.update('goalsList', "taskName = '%s', deadline = '%s'" % (self.tool.to_database(renamed_task_name), self.tool.to_database(deadline)), "taskName = '%s'" % self.tool.to_database(task_name))
                self.db.update('taskInfo', "taskName = '%s'"% self.tool.to_database(renamed_task_name), "taskName = '%s' AND listId = 2" % self.tool.to_database(task_name))
    ...
    def remove_task_yes(self, event):
        ...
        if self.list_id == 0:
            ...
            self.db.delete('yearList', "taskName = '%s'" % self.tool.to_database(task_name))
            self.db.delete('taskInfo', "taskName = '%s' AND listId = 0" % (self.tool.to_database(task_name)))
            self.task_info.delete(1.0, END)
            ...
            self.db.delete('dailiesList', "taskName = '%s'" % self.tool.to_database(task_name))
            self.db.delete('taskInfo', "taskName = '%s' AND listId = 1" % (self.tool.to_database(task_name)))
            self.task_info.delete(1.0, END)
            ...
            self.db.delete('goalsList', "taskName = '%s'" % self.tool.to_database(task_name))
            self.db.delete('taskInfo', "taskName = '%s' AND listId = 2" % (self.tool.to_database(task_name)))
            self.task_info.delete(1.0, END)
    ...
    def clear_task(self, event=""):
        ...
        clear_yearlies_button = Button(self.center_frame, text="Yearlies", width=10)
        clear_dailies_button = Button(self.center_frame, text="Dailies", width=10)
    ...
    def save_task_info(self, event=""):
        ...
        if self.list_id == 0:
            ...
            self.db.delete('taskInfo', "EXISTS (SELECT * FROM taskInfo WHERE taskName = '%s' AND listId = 0) AND taskName = '%s' AND listId = 0;"
                % (task_name, task_name))
            self.db.insert('taskInfo', 'listId, taskName, taskInfo', "0, '%s', '%s'" % (task_name, task_info))
            ...
            self.db.delete('taskInfo', "EXISTS (SELECT * FROM taskInfo WHERE taskName = '%s' AND listId = 1) AND taskName = '%s' AND listId = 1;"
                % (task_name, task_name))
            self.db.insert('taskInfo', 'listId, taskName, taskInfo', "1, '%s', '%s'" % (task_name, task_info))
            ...
            self.db.delete('taskInfo', "EXISTS (SELECT * FROM taskInfo WHERE taskName = '%s' AND listId = 2) AND taskName = '%s' AND listId = 2;"
                % (task_name, task_name))
            self.db.insert('taskInfo', 'listId, taskName, taskInfo', "2, '%s', '%s'" % (task_name, task_info))
...
class DayPlanner:
    def __init__(self):
        ...
        # Left Frame ====================
        left_frame = Frame(self.main_frame, bg="#333")
        hour_time_label = Label(left_frame, text='Hour', font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")
        activity_label = Label(left_frame, text='Overview', font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")

        self.hour_listbox = Listbox(left_frame, width=5, height=24, font=('Arial', 16), justify=CENTER)
        self.overview_listbox = Listbox(left_frame, width=50, height=24, font=('Arial', 16), justify=CENTER, exportselection=False)

        for i in range(0, 24):
            self.hour_listbox.insert(END, i)

        self.overview_listbox.bind('<<ListboxSelect>>', self.change_activity_list)
        ...
        # Right Frame ===================
        right_frame = Frame(self.main_frame, bg="#333")
        minute_time_label = Label(right_frame, text='Min', font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")
        minute_activity_label = Label(right_frame, text='Activities', font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")

        self.minute_listbox = Listbox(right_frame, width=5, height=12, font=('Arial', 16), justify=CENTER)
        self.activity_listbox = Listbox(right_frame, width=30, height=12, font=('Arial', 16), justify=CENTER, exportselection=False)
        ...
        self.add_button = Button(button_frame, text='Add', width=8, height=1, font=('Helvetica', 14))
        self.rename_button = Button(button_frame, text='Rename', width=8, height=1, font=('Helvetica', 14))
        self.remove_button = Button(button_frame, text='Remove', width=8, height=1, font=('Helvetica', 14))
        clear_button = Button(button_frame, text='Clear All', width=8, height=1, font=('Helvetica', 14))
        ...
        self.task_info_label = Label(right_frame, text='Activity: ', font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")
        self.task_info =  Text(right_frame, width=41, height=6, font=('Helvetica', 14))
        save_frame = Frame(right_frame, bg="#333")
        self.save_button = Button(save_frame, text='Save Info', width=8, height=1, font=('Helvetica', 14))
        self.save_indicator_label = Label(save_frame, text="", font=('Helvetica', 14), bg="#333", fg="#efefef")
    ...
    def initialize(self):
        self.db.raw_execute("SELECT DISTINCT hourIndex FROM activityList ORDER BY hourIndex ASC")
        ...
        for hour in hours:
            tasks_in_hour_list = []
            self.db.raw_execute("SELECT taskName FROM activityList WHERE hourIndex = %d ORDER BY minIndex ASC" % (hour))
    ...
    def change_activity_list(self, event):
        ...
        self.db.raw_execute("SELECT minIndex, taskName FROM activityList WHERE hourIndex = %d ORDER BY minIndex ASC" % (self.hour_id))
        activities = self.db.fetchall()

        for min_index, taskName in activities:
            self.activity_listbox.delete(min_index)
            self.activity_listbox.insert(min_index, self.tool.from_database(taskName))
    ...
    def change_task_info(self, event=""):
        ...
        try:
            ...
            self.db.select('activityInfo', 'taskInfo', 'hourIndex = %d AND minIndex = %d' % (self.hour_id, self.minute_id))
            task_info = self.db.fetchone()[0]
            self.task_info.insert(END, task_info)
    ...
    def save_task_info(self, event=""):
        try:
            ...
            self.db.insert('activityInfo', 'hourIndex, minIndex, taskInfo', "%d, %d, '%s'" % (self.hour_id, self.minute_id, task_info))
    ...
    def rename_task(self, event):
        try:
            ...
            self.rename_task_entry = Entry(self.rename_task_popup, text=task_name, width=30, justify=CENTER)
    ...
    def remove_task_yes(self, event):
        ...
        self.db.delete('activityList', "hourIndex = %d AND minIndex = %d" % (int(self.hour_id), int(selected_index)))
        self.db.delete('activityInfo', "hourIndex = %d AND minIndex = %d" % (int(self.hour_id), int(selected_index)))
    ...
    def clear_task(self, event):
        ...
        remove_task_label = Label(self.clear_task_popup, text="Are you sure you would like to clear all activities?", bg="#333", foreground="#efefef", font=('Arial', 13))
...
class LearntList:
    def __init__(self):
        ...
        grateful_label = Label(top_frame, text="What I Learnt / Achieved For The Day", font=('Helvetica', 17, 'bold'), bg="#333", fg="#efefef")

        self.number_listbox = Listbox(top_frame, width=3, height=20, font=('Arial', 15), justify=CENTER, exportselection=False)

        self.list_listbox = Listbox(top_frame, width=45, height=20, font=('Arial', 15), justify=CENTER, exportselection=False)
        ...
        add_button = Button(bottom_frame, text="Add", font=('Arial', 14), width=button_width)
        add_button.grid(row=0, padx=(0,20))

        remove_button = Button(bottom_frame, text="Remove Latest", font=('Arial', 14), width=button_width)
        remove_button.grid(row=0, padx=(0,20), column=1)

        clear_button = Button(bottom_frame, text="Clear", font=('Arial', 14), width=button_width)
    ...
    def add_item(self, event=""):
        ...
        add_item_label = Label(self.center_frame, text="Add Item", font=("Arial", 14), bg="#333", fg="#efefef")
        self.add_item_entry = Entry(self.center_frame, width=50, justify=CENTER)
    ...
    def add_item_to_app(self, event=""):
        ...
        if count == None:
            ...
            self.db.insert("gratList", "listId, gratInfo", "%d, \"%s\"" % (int(count) + 1, info))
...
class Database:
    def __init__(self, database):
        ...
        try:
            # To Do List
            self.cursor.execute("CREATE TABLE IF NOT EXISTS yearList(taskId INT, taskName VARCHAR(40));")
            ...
            # Gratitude List
            self.cursor.execute("CREATE TABLE IF NOT EXISTS gratList(listId INT, gratInfo VARCHAR(200));")
...
if __name__ == "__main__":
    ...
    menubar.add_command(label="Start Screen", command=start_screen, font=('Arial', 17))
    menubar.add_command(label="To-Do List", command=to_do_list, font=('Arial', 17))
    menubar.add_command(label="Day Planner", command=day_planner, font=('Arial', 17))
    menubar.add_command(label="Learnt List", command=gratitude_list, font=('Arial', 17))
